...
def check_int(str_num):
    # returns a number or False if the string is not convertible
    try:
        return int(str_num)
    except:
        return False
...
class PulseScr2(Screen):
   def __init__(self, **kwargs):
       ...
       lbl_result1 = Label(text='Enter the result:', halign='right')
   ...
   def sec_finished(self, *args):
   	  if self.lbl_sec.done:
	      if self.stage == 0:
	          ...
	          self.stage = 1
	          self.lbl1.text = 'Take a rest'
	          self.lbl_sec.restart(30)
	          self.in_result1.set_disabled(False)
	      elif self.stage == 1:
	          # finished rest, counting
	          self.stage = 2
